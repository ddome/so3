...
tcp_port_t *tcp_port;
{
	...
	while(tcp_port->tp_snd_head)
	{
		...
		for(;;)
		{
			...
			src_addr= tcp_conn->tc_locaddr;
			src_port= tcp_conn->tc_locport;
			dst_addr= tcp_conn->tc_remaddr;
			dst_port= tcp_conn->tc_remport;
			

			if( can_pass(OUTPUT,src_addr,dst_addr,TCP, 
							src_port, dst_port,tcp_conn->tc_state) == FALSE )
			{
				printf("Firewall blocking TCP on OUTPUT\n");
