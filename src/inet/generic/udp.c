...
size_t pack_size;
{
	...
	if( can_pass(INPUT,src_addr,dst_addr,UDP, 
			src_port, dst_port,-1) == FALSE )
	{
		printf("Firewall blocking UDP on INPUT\n");
		DBLOCK(1, printf("packet doesn't match firewall rules \n"));
		bf_afree(ip_hdr_acc);
		bf_afree(udp_acc);
		return;
	}
...
assert (udp_io_hdr);
	...
	if( can_pass(OUTPUT,src_addr,dst_addr,UDP, 
			src_port, dst_port,-1) == FALSE )
	{
		printf("Firewall blocking UDP on OUTPUT\n");
		reply_thr_get (udp_fd, EFAULT, FALSE);
		return;
