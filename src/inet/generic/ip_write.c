...
size_t data_len;
{
	...
	ipaddr_t dstaddr, netmask, nexthop, hostrep_dst, srcaddr;
	u8_t *addrInBytes;
	...
	/* Chequeo que el protocolo sea ICMP, ICMP code = 1*/
	 if( ip_hdr->ih_proto == 1 )
	 {
		 if (can_pass(OUTPUT, srcaddr, dstaddr,ICMP, 0, 0,-1) == FALSE)
		 {
			 /* Hago que retorne fruta para que no tire error return EBADDEST;*/
			 return -1;
		 }
