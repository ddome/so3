...
ioreq_t req;
{
	...
	case NWIOFIREWALL:
		data = (*ip_fd->if_get_userdata)(ip_fd->if_srfd,
			0, sizeof(nwio_firewall_t), TRUE);
		if (data == NULL)
		{
			return (*ip_fd->if_put_userdata)(ip_fd->if_srfd,
				EFAULT, NULL, TRUE);
		}
		data = bf_packIffLess(data, sizeof(nwio_firewall_t));
		firewall = (nwio_firewall_t *)ptr2acc_data(data);

		switch (firewall->opcode)
		{
			case ADD_RULE:
				add_rule(firewall->tabla, firewall->campos);
				break;
			case LIST_RULE:
				list_rules(firewall->tabla);
				break;
			case SAVE:
				printf("Guardar reglas\n");
				break;
			case DELETE:
				del_rule(firewall->campos.id);
				break;
			case DELETE_RULE:
				flush_table(firewall->tabla);
				break;
			case CHANGE_POLICY:
				change_policy(firewall->campos.action);
				break;
			case NONE:
				printf("Me llego none\n");
				break;
			default:
				printf("entre en el default\n");
				break;
		}
